...
export default function ManualInputForm({ onSubmit, onResult, language }: ManualInputFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // const response = await fetch("http://localhost:5000/predict-from-features", {
      //   method: "POST",
      //   headers: {
      ...
      setTimeout(() => {
        // Determine prediction based on jitter and shimmer values
        const jitterAvg = (features["MDVP:Jitter(%)"] + features["Jitter:DDP"]) / 2
        const shimmerAvg = (features["MDVP:Shimmer"] + features["Shimmer:APQ3"]) / 2
        const isParkinsons = jitterAvg > 0.6 && shimmerAvg > 0.6

        const mockResult = {
          prediction: isParkinsons ? "positive" : "negative",
          probability: isParkinsons ? 0.83 : 0.27,
          message: isParkinsons
            ? language === "en"
              ? "Indicators of Parkinson's disease detected"
              : "Indicateurs de la maladie de Parkinson détectés"
            : language === "en"
              ? "No significant indicators of Parkinson's disease detected"
              : "Aucun indicateur significatif de la maladie de Parkinson détecté",
          features: features,
        }
        onResult(mockResult)
      }, 2000)
      ...
      alert(language === "en" ? "An error occurred during analysis" : "Une erreur s'est produite lors de l'analyse")
      onResult(null)
    }
  }
      ...
      {showPresets && (
        <div className="mb-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          >
            ...
            <div className="text-[8.9px] text-muted-foreground">{t.parkinsonsPreset}</div>
            </div>
          </Button>
        </div>
      )}
